...
public class ProdutoRepository {
	...
	private static final String GET_ALL = "SELECT * FROM TB_PRODUTO P INNER JOIN TB_CATEGORIA C ON P.ID_CATEGORIA = C.ID_CATEGORIA "
			+ "INNER JOIN TB_MARCA M ON P.ID_MARCA = M.ID_MARCA ORDER BY P.ID";
	...
	private static final String GET = "SELECT * FROM TB_PRODUTO P INNER JOIN TB_CATEGORIA C  ON P.ID_CATEGORIA = C.ID_CATEGORIA "
			+ "INNER JOIN TB_MARCA M ON P.ID_MARCA = M.ID_MARCA WHERE ID=?";
	...
	public void save(ProdutoModel produtoModel) {
		this.jdbcTemplate.update(SAVE, produtoModel.getNome(), produtoModel.getSku(), produtoModel.getDescricao(), produtoModel.getPreco(), 
				produtoModel.getCaracteristicas(), produtoModel.getCategoriaModel().getIdCategoria(), produtoModel.getMarcaModel().getIdMarca());
	}
